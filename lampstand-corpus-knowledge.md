...
source: lampstand-corpus repo export (0a35656, 2026-07-02)
updated: 2026-07-02
...
> Knowledge export of the `lampstand-corpus` repository — the offline text-and-ML pipeline that builds the read-only Bible-study assets consumed by the [[lampstand-ios]] app. Synthesized from README, docs, the corpus manifest, source layout, and validation reports. No source code, secrets, or credentials are reproduced here.
...
- **Pipeline stages (the P0–P8 phase model).** Documented in `docs/normalized-schema.md` and `__init__.py`:
  ...
  4. **P3 commentaries** — parse CCEL/OCR commentaries → `commentaries.sqlite`.
...
- **`corpus_version`** — e.g. `corpus-v2.1.1-candidate`. **`ship_ready: false`** always — the pipeline never self-marks ship-ready; the architect's 23-point spot-check gates the ship tag.
...
- **`packs`** grouped by delivery channel:
  ...
  - **`on_demand`** (downloaded on first launch, free): `ondemand_bibles.sqlite` (KJV/ASV/WEB), `ondemand_commentaries.sqlite`, `ondemand_confessions.sqlite` (the other 6 documents), `ondemand_crossrefs.sqlite`, `ondemand_lexicons.sqlite`, `ondemand_search.sqlite` (full BM25 + metadata + display text; ~276 MB), `ondemand_vectors.sqlite` (int8 dense vectors; ~86 MB). ~610 MB total.
...
- **Commentaries (`sources/commentaries/`).** Four public-domain sets, chunked per paragraph:
  ...
  - **Charles Spurgeon — Treasury of David** (Psalms) — Internet Archive DjVu **OCR** (a *candidate*, not ship-ready), because the CCEL edition is image-only. All seven volumes: six from the Google-digitized `*spurgoog` scans, with Psalms 104–118 gap-filled from an alternate PD scan.
...
- **Spurgeon Treasury of David is OCR** — a candidate, not ship-ready; the underlying text is PD but OCR quality is a data-quality risk (98 OCR flags in validation).
...
- **Embedding index (P6).** From `output/embeddings.sqlite`: **317,204 chunks** embedded (commentary 145,581 · scripture 138,167 · lexicon 32,561 · confession 895), plus a BM25 index (~145k–174k terms, ~10.8M–14.2M postings depending on corpus version). 3/3 cosine smoke queries pass. Model: BGE-small-en-v1.5, dim 384, pinned HF revision `5c38ec7c405e`.
...
- **Downstream integration.** The [[lampstand-ios]] app (private repo) is the sole consumer. Integration is **file-based, not networked**: the app reads `corpus_manifest.json`, syncs the pack `.sqlite` and model `.mlpackage` files as checksummed assets, and verifies each `sha256`. The app's reader contracts are pinned in `docs/pack-diet.md`, `docs/crossrefs-pack.md`, `docs/reranker-pack.md`, and the re-sync checklist `docs/pack-handoff-v2.1.md`.
...
- **Reranker → export.** The rerank gate verdict is SHIP; next step is CoreML export + app wiring (largely done as of `corpus-v2.1.1-candidate`).
- **Size levers (carried into the freeze).** `ondemand_search.sqlite` (~276 MB) exceeds the 250 MiB soft target — the next lever (architect decision) is to drop child display text and resolve from the per-resource packs. `Reranker.mlpackage` (~43.4 MB fp16) exceeds the ~20–25 MB back-of-envelope; the binary-size budget was architect-confirmed.
- **Ship gate.** `ship_ready = false` on every candidate; the architect's **23-point spot-check** gates the version tag. The pipeline structurally never self-marks ship-ready.
...
- **Data quality — OCR.** Spurgeon's Treasury of David is DjVu OCR (98 OCR flags); it is explicitly a *candidate*, and OCR artifacts are the biggest text-fidelity risk in the corpus.
...
- **Human-gated ship is a process dependency.** Nothing ships without the architect's manual 23-point spot-check + a theological advisor approving the DRAFT eval files. This is intentional theological care, but it is a human bottleneck and a single point of judgment.
- **Two-repo drift.** The corpus and [[lampstand-ios]] are separate lanes coordinated only by `corpus_manifest.json` + the handoff docs. A reader-contract change (e.g. the parent-text-NULL migration) requires coordinated app-side work; skew between the emitted packs and the app's reader is a live risk (the handoff docs exist precisely to manage it).
...
- **Bundled vs on-demand** — bundled packs ship inside the app binary (public-domain/CC0 only); on-demand packs download on first launch.
...
- **Candidate vs ship-ready** — the pipeline only ever emits a *candidate*; `ship_ready=false` until the architect's 23-point human spot-check passes.
...
Ingest note: drop into the Obsidian vault Clippings/ and ask OpenClaw to ingest.

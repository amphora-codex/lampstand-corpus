...
def render_report(gold: dict, results: dict, sweep: dict | None) -> str:
    ...
    w.append(f"| hardneg | {counts.get('hardneg', 0)} | WSC/WLC question A | A's "
             "Q&A chunk (hard negative: doctrinally-adjacent B's chunk) — "
             "**architect-approved** (`data/eval/hard_negatives_v1.json`; advisor "
             "spot-check still recommended) |")
    w.append("")
    w.append("> **Labels-grew caveat.** The Belgic Confession's proof-texts were "
             "added to `confessions.sqlite` from CCEL schaff/creeds3, so the "
             "prooftext gold set now includes Belgic articles (6 Belgic queries; "
             "prooftext n 149 → 150). More labels usually LOWER apparent scores "
             "(the denominator grows and the new queries are hard corpus-native "
             "proof-text queries), so a small dip here is EXPECTED and is NOT a "
             "retrieval regression — the retriever, packs, and fusion constants "
             "are byte-identical to the prior run; only the measured label set "
             "changed.")
    w.append("")
    ...
    w.append("")
    w.append("- Hard-negative suite (`data/eval/hard_negatives_v1.json`) is "
             "**architect-approved** (2026-07-02); a theological-advisor "
             "spot-check is still recommended before public ship, since pairs "
             "were generated by answer token overlap and some may be "
             "doctrinally trivial or too close.")
    w.append("- The prooftext category now draws from WCF / WLC / WSC / LBCF / "
             "Heidelberg / Dort / Belgic (Belgic proofs added from CCEL "
             "schaff/creeds3: 34/37 articles, arts 4-6 the canon list carry none).")
    for note in gold.get("notes", []):
        if "DRAFT" in note or "only" in note or "recommended" in note:

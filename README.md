...
- **Lexicons:** Strong's Greek + Hebrew dictionaries (`openscriptures/strongs`, CC-BY-SA edition / PD text) and Brown-Driver-Briggs (`openscriptures/HebrewLexicon`, CC-BY-4.0 edition / PD text, keyed to Strong's via the repo's `LexicalIndex.xml`). **Thayer's** has no clean canonical machine-readable PD source and is flagged for human sourcing (omitted, not fabricated). Strong's-tagged original text: the OSHB Hebrew (`openscriptures/morphhb`, CC-BY-4.0) is ingested per-word; **MorphGNT/SBLGNT** Greek is snapshotted for provenance but deferred — it carries no Strong's numbers and the SBLGNT text is under the SBLGNT EULA (bundling is an architect decision).
